...
if __name__ == "__main__":
    folder = 1
    xy_path = '../test/xy.csv'.format(folder)  # 保存的四个像素坐标，x、y
    params_path = '../test/params.csv'  # matlab计算后得到的params.csv内参
    res_path = '../test/world_params.csv'.format(folder)  # 计算外参后生成的world_params.csv文件，存储相机内外参数
